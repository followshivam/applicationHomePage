import { NotFound, useTranslation, Pagination, SearchBox, SelectBox, Spinner, TableComponent, Button, DialogActions, Divider, makeStyles, Typography } from "component";
...
const SelectReport = props => {
  ...
  const [inputParameters, setInputParameters] = useState({ reportType: 0, filter: "" });
  ...
  const GetCategoryListData = () => {
    GetCategoryList({ "screenid": "ReportDesigner" }).then(response => {
      if (response != null && response.status != null && response.status.maincode === "0") {
        let additionalCategories = response.data.category.map((category, index) => ({
  ...
  const GetDataFromApi = (opr = "0", last_index = "", last_name = "") => {
    ...
    if (reportList != null && reportList[inputParameters.reportType] != null) {
    ...
    GetReportList(payload).then(response => {
      if (response != null && response.status != null && response.status.maincode === "0") {
  ...
  const onNextHandler = event => {
    ...
    if (selected != null)
      dispatch(CreateAddReport({ ...createAddReportState, current_step: createAddReportState.current_step + 1, ...selected, ...data }))
    else
      snackbarState.openSnackbar(`${t('bam:SELECT_A_REPORT')}`, 'warning')
  }

  const onSearchSubmit = (obj) => {
    let param = obj.searchString.trim();
    if (param !== '')
      setInputParameters({
        ...inputParameters,
        filter: param,
    ...
    <div className={classes.body}>
      ...
      <div className={classes.toolbar}>
        <div className={classes.toolbarText}>
          <Typography variant="subtitle1" className={classes.textColor}>{t('bam:SELECT_A_REPORT_TO_ADD')}:</Typography>
        </div>
        <div className={classes.toolbarControls}>
          ...
          />
          <SelectBox list={reportList}
            value={inputParameters.reportType}
            className={classes.inputBox}
            onChange={event => setInputParameters({ ...inputParameters, reportType: event.target.value })}
          />
          <Pagination disabled_next={apiData != null ? !apiData.enable_next : true} disabled_prev={apiData != null ? !apiData.enable_prev : true} onChange={handlePagination} />
        </div>
      </div>
      <Divider fullWidth />
      <div className={classes.tableContainer}>
        {isLoading.loading === false ?
          (apiData != null && apiData.reports.length > 0 ?
            <TableComponent dynamicHeight="220px"
      ...
      <DialogActions className={classes.actionBar}>
        <Button variant="outlined" onClick={props.onCancel}>{t('bam:CANCEL')}</Button>
